...
class PeriodFilter {
    ...
    handleSearch() {
        ...
        let selectElement = document.getElementById("industry-select");
    ...
    performSearch(searchData) {
        ...
        .then(blob => {
           // Из blob создаем URL
            ...
            imgElement.src = "static/plot2.png?" + + new Date().getTime(); // Кладем src
            imgElement.alt = "static/plot2.png";
            document.body.appendChild(imgElement); // Добавляем на страницу
        ...
        fetch('/get_revenue', {
         ...
         .then(data => {
			 ...
			 document.getElementById('stat-value-work').textContent = data.sum3 + ' кол-во человек';
...
//           // Из blob создаем URL
//            const imgElement = document.createElement('img');
//            imgElement.src = "static/plot.png"; // Кладем src
//            imgElement.alt = "static/plot.png";
//            document.body.appendChild(imgElement); // Добавляем на страницу
//            console.log("добавил картинку")
        })
        .catch(error => {
 ...
 */
function initIndustrySelect(selectId = 'industry-select', apiUrl = '/all-industries') {
    const selectElement = document.getElementById(selectId);
    let selectElement2 = selectElement;
    ...
    // Загружаем данные с сервера
        ...
        body: JSON.stringify({
            industry: selectElement2 // например, 'IT'
        })
            })
            .then(response => {
                if (!response.ok) {
                ...
                console.log(response.json.industries)
...
function updateSelectOptions(selectElement, industries) {
    ...
    console.log(Array.isArray(industries))
    console.log(industries[0]);
    for (let i = 0; i < industries.industries.length; i++) {
    const option = document.createElement('option');
    option.value = industries.industries[i]
    option.textContent = industries.industries[i]
